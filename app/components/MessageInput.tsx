...
export default function MessageInput() {
    ...
    const handleSendMessage = (key: string, type: string) => {
      if (text && key === 'Enter' || text && type === 'click') {
    ...
    return (
      <div className="bottom-0 relative my-2.5 w-full px-5 sm:px-24 lg:px-52">
        <div className="relative">
          <textarea className="w-full text-sm rounded-xl p-3.5 border-indigo-monta border" name="Message" placeholder="Message" rows={1} value={text} onChange={(e) => setText(e.target.value)} onKeyUp={(e) => handleSendMessage(e.key, '')}></textarea>
          <div className="absolute cursor-pointer text-xl top-3.5 right-8">
            <FontAwesomeIcon title="Send" icon={faPaperPlane} onClick={(e) => handleSendMessage('', e.type)} />
          </div>
        </div>
      </div>
    );
